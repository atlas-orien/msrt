...
`EngineState` 不是一个“大 machine”的抽象名字，而是多个内部状态机的聚合。它不直接保存裸的 packet number、message id、dedup table、decoder buffer 或 event queue，而是把这些状态收进对应的 state 对象：

- `ClockState` 负责保存 engine 当前看到的单调时间。
- `NumberState` 负责分配 packet number 和 message id。
- `SchedulerState` 负责输出动作优先级、去重和排队。
- `AckState` 负责 ACK 观察、ACK range 和 ACK pending 状态。
