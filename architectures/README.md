...
但是 v1 的目标是可靠传输，不只是协议骨架。当前已经补齐 ACK range 最小版本、重试失败、多 message / 多 channel reassembly 和 BestEffort 最小策略。下一步继续按 [v1 reliable transport plan](v1/srt-reliable-transport-plan.md) 收敛 ACK range 压缩和过期策略、message 失败清理语义、LatestOnly / Deadline、buffer 策略最终冻结和更长时间持续收发验收测试。
