...
const PACKET_NUMBER_LEN: usize = core::mem::size_of::<u32>();
const MESSAGE_ID_LEN: usize = core::mem::size_of::<u32>();
const MESSAGE_LEN_LEN: usize = core::mem::size_of::<u16>();
const FRAGMENT_OFFSET_LEN: usize = core::mem::size_of::<u16>();
const FRAGMENT_FLAGS_LEN: usize = core::mem::size_of::<u8>();
const FRAGMENT_HEADER_LEN: usize =
    MESSAGE_ID_LEN + MESSAGE_LEN_LEN + FRAGMENT_OFFSET_LEN + FRAGMENT_FLAGS_LEN;
const PACKET_META_LEN: usize = PACKET_NUMBER_LEN + FRAGMENT_HEADER_LEN;

/// Maximum encoded wire bytes held by one MVP endpoint event.
pub const MAX_WIRE_BYTES: usize = 128;
/// Maximum complete message bytes held by the MVP endpoint facade.
pub const MAX_MESSAGE_BYTES: usize = 256;
/// Maximum pending endpoint events.
pub const MAX_EVENTS: usize = 16;
/// Default maximum message fragment bytes per packet.
pub const DEFAULT_FRAGMENT_BYTES: usize = 12;

const FRAGMENT_FIRST: u8 = 1 << 0;
const FRAGMENT_LAST: u8 = 1 << 1;

/// Endpoint configuration.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EndpointConfig {
    /// First packet number used by this endpoint.
    pub initial_packet_number: PacketNumber,
    /// First message identifier used by this endpoint.
...
/// User-facing SRT endpoint facade.
///
/// This MVP facade is intentionally non-blocking. `send` queues protocol write
/// events, `receive` feeds already-arrived bytes, and `poll_event` lets the
/// embedding environment drain protocol outputs.
...
impl Endpoint {
    ...
    ///
    /// The caller submits the complete message once. The endpoint splits it
    /// into packet-sized write events internally.
    pub fn send(&mut self, message: &[u8]) -> Result<MessageId> {
        ...
        while offset < message.len() || (message.is_empty() && offset == 0) {
            ...
            self.events.push(EndpointEvent::Write(WriteEvent {
                packet_number,
                bytes: wire,
                len: written,
            }))?;
    ...
    /// Feeds already-arrived wire bytes into the endpoint.
    ///
        ...
        match decode_message_fragment(bytes, &Crc16) {
            FragmentDecode::Fragment(fragment) => {
                ...
                match self.reassembly.observe(fragment) {
                    Ok(Some(message)) => {
                        if self.events.push(EndpointEvent::Message(message)).is_err() {
    ...
    /// Polls one queued endpoint event.
    ...
    ///
    /// The MVP facade keeps this as a boundary for future ACK timeout and
...
/// Events produced by an endpoint.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// A non-blocking write request produced by the endpoint.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// A complete message delivered by the endpoint.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MessageEvent {
    /// Message identifier scoped to this endpoint.
...
pub enum ReceiveReport {
    ...
    /// The packet was valid but could not be applied to endpoint state.
    Error(Error),
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct EventQueue {
    events: [Option<EndpointEvent>; MAX_EVENTS],
    head: usize,
    len: usize,
}
...
impl EventQueue {
    ...
    fn push(&mut self, event: EndpointEvent) -> Result<()> {
    ...
    fn pop(&mut self) -> Option<EndpointEvent> {
        if self.len == 0 {
            return None;
        }
...
#[cfg(test)]
mod tests {
    use super::{Endpoint, EndpointConfig, EndpointEvent, ReceiveReport};

    #[test]
    fn endpoint_sends_one_message_as_multiple_write_events() {
        let mut endpoint = Endpoint::new(EndpointConfig {
            fragment_bytes: 5,
            ..EndpointConfig::default()
        });

        let message_id = endpoint.send(b"hello srt testing").unwrap();
        let mut writes = 0;

        while let Some(event) = endpoint.poll_event() {
            match event {
                EndpointEvent::Write(_) => writes += 1,
                EndpointEvent::Message(_) => panic!("sender should not receive its own message"),
            }
        }
    ...
    #[test]
    fn endpoint_receives_fragments_as_one_message_event() {
        let mut a = Endpoint::new(EndpointConfig {
            fragment_bytes: 5,
            ..EndpointConfig::default()
        });
        ...
        let Some(EndpointEvent::Message(message)) = b.poll_event() else {
