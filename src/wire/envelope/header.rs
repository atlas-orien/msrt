...
pub struct EnvelopeHeader {
    ...
    pub packet_len: u8,
    /// CRC-8 over the magic and packet length fields.
    pub header_crc: u8,
}
...
impl EnvelopeHeader {
    ...
    pub const fn new(packet_len: u8) -> Self {
        Self {
            ...
            packet_len,
            header_crc: header_crc(EnvelopeMagic::MSRT, packet_len),
        }
    }
    ...
    pub const fn has_valid_header_crc(self) -> bool {
        self.header_crc == header_crc(self.magic, self.packet_len)
    }
...
/// Calculates the CRC-8 over the protected wire header fields.
#[must_use]
pub const fn header_crc(magic: EnvelopeMagic, packet_len: u8) -> u8 {
    let bytes = [magic.bytes()[0], packet_len];
...
mod tests {
    ...
    fn header_crc_changes_when_length_changes() {
        assert_ne!(
            header_crc(EnvelopeMagic::MSRT, 9),
            header_crc(EnvelopeMagic::MSRT, 10)
        );
    }
}
